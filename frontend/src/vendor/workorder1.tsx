...
function WorkOrders() {
  ...
  const [storedVendorId , setstoredVendorId ] = useState<string>('0');

  useEffect(() => {
      const storedUser = localStorage.getItem('user');
      if (storedUser) {
        const parsedUser = JSON.parse(storedUser);
        setstoredVendorId(parsedUser?.user_id ?? '0');
      }
  }, []);
  ...
  const fetchTableData = async () => {
    ...
    try {
      ...
      params.append('vendor_id', storedVendorId); 
  ...
  return (
    <Box sx={{ p: 3 }}>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6} md={4}>
                  <TextField
                    ...
                    label="Vendor ID"
                    type="number"
                    value={storedVendorId || formData.vendor_id || ''}
                    aria-readonly={true}
                    disabled={true}
      ...
      {/* View Modal - keeping the existing one */}
