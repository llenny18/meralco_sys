// pages/sector-manager/dashboard.tsx
...
const SECTOR_MANAGER_TABLES: TableConfig[] = [
...
function SectorManagerDashboard() {
  const [selectedTable, setSelectedTable] = useState<string>('sectors');
  ...
  const handleDelete = async (row: any) => {
    ...
    try {
      const primaryKey = row.id || row.sector_id || row.project_id || row.vendor_id || row.invoice_id || row.penalty_id || row.delay_id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const primaryKey = currentRecord.id || currentRecord.sector_id || currentRecord.project_id || currentRecord.vendor_id;
  ...
  const getCurrentTableConfig = () => SECTOR_MANAGER_TABLES.find((t) => t.endpoint === selectedTable);
  ...
  return (
    <>
      <Head><title>Sector Manager Dashboard</title></Head>
      <PageTitleWrapper>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            <Typography variant="h3" component="h3" gutterBottom>Sector Manager Dashboard</Typography>
            <Typography variant="subtitle2">Strategic oversight of sectors, projects, vendors, and financial performance</Typography>
          </Grid>
        </Grid>
      </PageTitleWrapper>
      <Container maxWidth="lg">
        <Grid container direction="row" justifyContent="center" alignItems="stretch" spacing={3}>
          <Grid item xs={12}>
            <Card>
              <CardHeader action={<Button variant="contained" startIcon={<AddTwoToneIcon />} onClick={handleAdd}>Add New</Button>} title="Sector Management" />
              <Divider />
              <CardContent>
                <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                  <FormControl sx={{ minWidth: 350, flexGrow: 1 }}>
                    ...
                    <Select value={selectedTable} onChange={handleTableChange} label="Select Table">
                      {SECTOR_MANAGER_TABLES.map((table) => <MenuItem key={table.endpoint} value={table.endpoint}>{table.label}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Box>
...
SectorManagerDashboard.getLayout = (page) => <SidebarLayout>{page}</SidebarLayout>;
