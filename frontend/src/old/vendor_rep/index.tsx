...
const VENDOR_TABLES: TableConfig[] = [
...
function VendorDashboard() {
  ...
  const handleDelete = async (row: any) => {
    ...
    try {
      const primaryKey = row.id || row.project_id || row.invoice_id || row.payment_id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const primaryKey = currentRecord.id || currentRecord.project_id || currentRecord.invoice_id;
  ...
  const getCurrentTableConfig = (): TableConfig | undefined => {
    return VENDOR_TABLES.find((t) => t.endpoint === selectedTable);
  };
  ...
  return (
    <>
      <Head>
        <title>Vendor Dashboard - Project Management</title>
      </Head>
      <PageTitleWrapper>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            ...
            <Typography variant="subtitle2">
              Manage your projects, documents, invoices, and performance metrics
            </Typography>
          </Grid>
        </Grid>
      </PageTitleWrapper>
                            ...
                            <TableRow hover key={row.id || index}>
                              ...
                              <TableCell align="center">
                                <Tooltip title="Edit" arrow>
                                  <IconButton color="primary" size="small" onClick={() => handleEdit(row)} sx={{ mr: 1 }}>
            ...
            <Grid container spacing={2}>
              {tableConfig && tableConfig.columns.map((column) => {
                ...
                const isBoolean = typeof currentValue === 'boolean' || 
                  (currentRecord[column] !== undefined && typeof currentRecord[column] === 'boolean');
...
VendorDashboard.getLayout = (page) => <SidebarLayout>{page}</SidebarLayout>;
