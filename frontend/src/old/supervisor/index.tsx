// pages/wo-supervisor/dashboard.tsx
...
const WO_SUPERVISOR_TABLES: TableConfig[] = [
...
function WOSupervisorDashboard() {
  ...
  const handleDelete = async (row: any) => {
    ...
    try {
      const primaryKey = row.id || row.project_id || row.vendor_id || row.penalty_id || row.invoice_id || row.inspection_id || row.escalation_id || row.workflow_id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const primaryKey = currentRecord.id || currentRecord.project_id || currentRecord.vendor_id;
  ...
  const getCurrentTableConfig = () => WO_SUPERVISOR_TABLES.find((t) => t.endpoint === selectedTable);
  ...
  return (
    <>
      <Head><title>WO Supervisor Dashboard</title></Head>
      <PageTitleWrapper>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            <Typography variant="h3" component="h3" gutterBottom>WO Supervisor Dashboard</Typography>
            <Typography variant="subtitle2">Oversee all projects, vendors, penalties, SLA compliance, and inspections</Typography>
          </Grid>
        </Grid>
      </PageTitleWrapper>
      <Container maxWidth="lg">
        <Grid container direction="row" justifyContent="center" alignItems="stretch" spacing={3}>
          <Grid item xs={12}>
            <Card>
              <CardHeader action={<Button variant="contained" startIcon={<AddTwoToneIcon />} onClick={handleAdd}>Add New</Button>} title="Supervisor Operations" />
              <Divider />
              <CardContent>
                <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                  <FormControl sx={{ minWidth: 350, flexGrow: 1 }}>
                    ...
                    <Select value={selectedTable} onChange={handleTableChange} label="Select Table">
                      {WO_SUPERVISOR_TABLES.map((table) => <MenuItem key={table.endpoint} value={table.endpoint}>{table.label}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Box>
...
WOSupervisorDashboard.getLayout = (page) => <SidebarLayout>{page}</SidebarLayout>;
export default WOSupervisorDashboard;