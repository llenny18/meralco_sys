...
const CLERK_TABLES: TableConfig[] = [
...
function ClerkDashboard() {
  ...
  const handleDelete = async (row: any) => {
    ...
    try {
      const primaryKey = row.id || row.project_id || row.notification_id || row.log_id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const primaryKey = currentRecord.id || currentRecord.project_id || currentRecord.notification_id;
  ...
  const getCurrentTableConfig = (): TableConfig | undefined => {
    return CLERK_TABLES.find((t) => t.endpoint === selectedTable);
  };
  ...
  return (
    <>
      <Head>
        <title>Clerk Dashboard - Project Management</title>
      </Head>
      <PageTitleWrapper>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            ...
            <Typography variant="subtitle2">
              Manage projects, documents, compliance, and notifications
            </Typography>
          </Grid>
        </Grid>
      </PageTitleWrapper>
...
ClerkDashboard.getLayout = (page) => <SidebarLayout>{page}</SidebarLayout>;
