...
const ENGINEER_TABLES: TableConfig[] = [
...
function EngineerDashboard() {
  ...
  const handleDelete = async (row: any) => {
    ...
    try {
      const primaryKey = row.id || row.project_id || row.workflow_id || row.escalation_id || row.delay_id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const primaryKey = currentRecord.id || currentRecord.project_id || currentRecord.workflow_id;
  ...
  const getCurrentTableConfig = () => ENGINEER_TABLES.find((t) => t.endpoint === selectedTable);
  ...
  return (
    <>
      <Head><title>Engineer Dashboard</title></Head>
      <PageTitleWrapper>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            <Typography variant="h3" component="h3" gutterBottom>Engineer Dashboard</Typography>
            <Typography variant="subtitle2">Manage projects, workflows, SLA tracking, and escalations</Typography>
          </Grid>
        </Grid>
      </PageTitleWrapper>
      <Container maxWidth="lg">
        <Grid container direction="row" justifyContent="center" alignItems="stretch" spacing={3}>
          <Grid item xs={12}>
            <Card>
              <CardHeader action={<Button variant="contained" startIcon={<AddTwoToneIcon />} onClick={handleAdd}>Add New</Button>} title="Engineering Management" />
              <Divider />
              <CardContent>
                <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                  <FormControl sx={{ minWidth: 350, flexGrow: 1 }}>
                    ...
                    <Select value={selectedTable} onChange={handleTableChange} label="Select Table">
                      {ENGINEER_TABLES.map((table) => <MenuItem key={table.endpoint} value={table.endpoint}>{table.label}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Box>
...
EngineerDashboard.getLayout = (page) => <SidebarLayout>{page}</SidebarLayout>;
