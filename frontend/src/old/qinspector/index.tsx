// pages/quality-inspector/dashboard.tsx
...
function QualityInspectorDashboard() {
  const [selectedTable, setSelectedTable] = useState<string>('qi-inspections');
  ...
  const handleDelete = async (row: any) => {
    ...
    try {
      const primaryKey = row.id || row.inspection_id || row.project_id || row.inspection_type_id;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const primaryKey = currentRecord.id || currentRecord.inspection_id || currentRecord.project_id;
  ...
  const getCurrentTableConfig = () => QI_TABLES.find((t) => t.endpoint === selectedTable);
    ...
    <>
      ...
      <PageTitleWrapper>
        <Grid container justifyContent="space-between" alignItems="center">
          <Grid item>
            <Typography variant="h3" component="h3" gutterBottom>Quality Inspector Dashboard</Typography>
            <Typography variant="subtitle2">Manage inspections, targets, and performance metrics</Typography>
          </Grid>
        </Grid>
      </PageTitleWrapper>
      <Container maxWidth="lg">
        <Grid container direction="row" justifyContent="center" alignItems="stretch" spacing={3}>
          <Grid item xs={12}>
            <Card>
              <CardHeader action={<Button variant="contained" startIcon={<AddTwoToneIcon />} onClick={handleAdd}>Add New</Button>} title="QI Operations" />
              <Divider />
              <CardContent>
                <Box sx={{ display: 'flex', gap: 2, mb: 3, flexWrap: 'wrap' }}>
                  <FormControl sx={{ minWidth: 350, flexGrow: 1 }}>
                    ...
                    <Select value={selectedTable} onChange={handleTableChange} label="Select Table">
                      {QI_TABLES.map((table) => <MenuItem key={table.endpoint} value={table.endpoint}>{table.label}</MenuItem>)}
                    </Select>
                  </FormControl>
                </Box>
...
QualityInspectorDashboard.getLayout = (page) => <SidebarLayout>{page}</SidebarLayout>;
