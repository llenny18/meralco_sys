...
const COLUMNS = ['username', 'email', 'role', 'is_active', 'phone_number'];
...
function UsersTable() {
  ...
  const handleEdit = (row: any) => { 
    ...
    // If role is an object with role_id, extract the ID
    const roleId = typeof row.role === 'object' && row.role !== null ? row.role.role_id : row.role;
    setFormData({ ...row, role: roleId }); 
    setShowModal(true); 
  };
  ...
  const getRoleName = (roleValue: any) => {
    if (!roleValue) return '-';
    // If role is an object with role_name
    if (typeof roleValue === 'object' && roleValue.role_name) {
      return roleValue.role_name;
    }
    // If role is an ID, find the role name
    const role = roles.find(r => r.role_id === roleValue);
    return role ? role.role_name : roleValue;
  };
    ...
    <>
      ...
      <Container maxWidth="lg">
        <Grid container direction="row" justifyContent="center" alignItems="stretch" spacing={3}>
          <Grid item xs={12}>
            <Card>
              <CardHeader action={<Button variant="contained" startIcon={<AddTwoToneIcon />} onClick={handleAdd}>Add New</Button>} title="Users Management" />
              <Divider />
              <CardContent>
                        ...
                        <TableBody>
                          {paginatedData.map((row, index) => (
                            <TableRow hover key={row.id || index}>
                              {COLUMNS.map((column) => <TableCell key={column}>{renderCellValue(column, row[column])}</TableCell>)}
          ...
          <Box component="form" onSubmit={handleSubmit}>
            <Grid container spacing={2}>
              {COLUMNS.map((column) => {
                ...
                // Special handling for role field - use select box
                if (column === 'role') {
                  return (
                ...
                // Boolean fields
                const isBoolean = column === 'is_active' || typeof currentValue === 'boolean' || (currentRecord[column] !== undefined && typeof currentRecord[column] === 'boolean');
                return (
                  <Grid item xs={12} sm={6} key={column}>
                    {isBoolean ? (
                      <FormControl fullWidth>
...
UsersTable.getLayout = (page) => <SidebarLayout>{page}</SidebarLayout>;
